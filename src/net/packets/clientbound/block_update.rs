use crate::net::codec::{write_var, write_position};
...
pub async fn broadcast_block_update(x: i32, y: i32, z: i32, block_id: i32) -> anyhow::Result<()> {
    ...
    for (uuid, pos) in player_positions.iter() {
        ...
        if distance_squared < (view_distance_blocks as f64).powi(2) {
            let tx = crate::server::conn::PLAYER_SOCKET_MAP.read().await.get(uuid).cloned().unwrap();
...
pub async fn send_block_update<W: tokio::io::AsyncWriteExt + Unpin>(stream: &mut W, x: i32, y: i32, z: i32, block_id: i32) -> anyhow::Result<()> {
