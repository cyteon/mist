...
use crate::types::colors::{GREEN, YELLOW, RED};
...
pub async fn handle_command(command: String, player: &mut Player) -> anyhow::Result<()> {
    ...
    match command_parts[0] {
        "tps" => {
            ...
            let tps_5s_color = if tps_5s >= 18 { GREEN } else if tps_5s >= 15 { YELLOW } else { RED };
            let tps_1m_color = if tps_1m >= 18 { GREEN } else if tps_1m >= 15 { YELLOW } else { RED };
            let tps_5m_color = if tps_5m >= 18 { GREEN } else if tps_5m >= 15 { YELLOW } else { RED };
